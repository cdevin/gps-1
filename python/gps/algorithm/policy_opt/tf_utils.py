...
class TfSolver:
    ...
    def __init__(self, loss_scalar, solver_name='adam', base_lr=None, lr_policy=None,
                 momentum=None, weight_decay=None, fc_vars=None, last_conv_tensors=None):
    ...
    def get_last_conv_values(self, sess, feed_dict, num_values, batch_size):
        ...
        while i < num_values:
            ...
            batch_vals = sess.run(self.last_conv_tensors, batch_dict)
            values.append(batch_vals)
            i = end
        final_values = [None for v in self.last_conv_tensors]
        i = 0

        for v in range(len(self.last_conv_tensors)):
