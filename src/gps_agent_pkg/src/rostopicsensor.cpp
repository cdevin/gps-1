...
ROSTopicSensor::ROSTopicSensor(ros::NodeHandle& n, RobotPlugin *plugin): Sensor(n, plugin)
{
    ...
    data_size_ = 64; // 32*2 for pos and 32*2 for vel
    latest_data_.resize(data_size_*2);
    latest_data_eigen_.resize(data_size_*2);
    previous_data_.resize(data_size_);
    cached_data_.resize(data_size_);
    previous_data_t_ = 0;
...
void ROSTopicSensor::update_data_vector(const std_msgs::Float64MultiArray::ConstPtr& msg) {
	...
	assert(latest_data_.size() == data_size_*2);
    ...
    /*    for (int i = 64; i < data_size_; i++)
	{
	    ...
	    latest_data_[i] = latest_data_[i-64]-msg->data[i-64];
	    latest_data_eigen_[i] =latest_data_eigen_[i-64] -msg->data[i-64];
	}
    */
    for (int i = 0; i < data_size_; i++)
	{
	    ...
	    //previous_data_[i] = latest_data_[i];
...
void ROSTopicSensor::set_sample_data_format(boost::scoped_ptr<Sample>& sample) 
{
    ...
    sample->set_meta_data(gps::IMAGE_FEAT,data_size_*2,SampleDataFormatEigenVector,data_metadata);
}
// Set data on the provided sample.
void ROSTopicSensor::set_sample_data(boost::scoped_ptr<Sample>& sample, int t) 
{
  if (t> previous_data_t_) {
    for (int i = 0; i < 64; i++)
      {
	previous_data_[i] = cached_data_[i];
	cached_data_[i] = latest_data_[i];
      }
    previous_data_t_ = t;
  }
  ...
  for (int i = 0; i < 64; i++)
    {
      if (t==0) { //(previous_data_.empty()) {
	//ROS_INFO("t is %d, previous data is empty", t);
	latest_data_[i+64] =0; //latest_data_[i]-previous_data_[i];
	//latest_data_eigen_[i] =0;// latest_data_eigen_[i]-previous_data_[i];
	previous_data_t_ = 0;
	cached_data_[i] = latest_data_[i]; 
     } else {
	//ROS_INFO("t is %d, previous_t is %d, diff is %e", t, previous_data_t_, latest_data_[i]-previous_data_[i]);
	latest_data_[i+64] =latest_data_[i]-previous_data_[i];
	//latest_data_eigen_[i] = latest_data_eigen_[i]-previous_data_[i];
      }
    }
